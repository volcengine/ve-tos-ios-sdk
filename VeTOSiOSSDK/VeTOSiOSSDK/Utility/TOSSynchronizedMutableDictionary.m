...
@interface TOSSynchronizedMutableDictionary()
...
@implementation TOSSynchronizedMutableDictionary
...
- (instancetype)syncedDictionary {
    TOSSynchronizedMutableDictionary *result = [[TOSSynchronizedMutableDictionary alloc] initWithDictionary:@{}.mutableCopy queue:self.dispatchQueue syncKey:self.syncKey];
    return result;
}
...
+ (void)mutateSyncedDictionaries:(NSArray<TOSSynchronizedMutableDictionary *> *)dictionaries withBlock:(void (^)(NSUUID *, NSMutableDictionary *))block {
    TOSSynchronizedMutableDictionary *first = [dictionaries firstObject];
    if (!first) { return; }

    dispatch_barrier_sync(first.dispatchQueue, ^{
        [dictionaries enumerateObjectsUsingBlock:^(TOSSynchronizedMutableDictionary * _Nonnull atomicDictionary, NSUInteger index, BOOL * _Nonnull stop) {

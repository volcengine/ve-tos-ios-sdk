...
@interface TOSExecutor ()

@property (nonatomic, copy) void(^block)(void(^block)(void));

@end

@implementation TOSExecutor

#pragma mark - Executor methods

+ (instancetype)defaultExecutor {
    static TOSExecutor *defaultExecutor = NULL;
...
+ (instancetype)immediateExecutor {
    static TOSExecutor *immediateExecutor = NULL;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
+ (instancetype)mainThreadExecutor {
    static TOSExecutor *mainThreadExecutor = NULL;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
